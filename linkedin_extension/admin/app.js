...
function initializePortal() {
  chrome.storage.local.get(["auth_token", "username"], async (result) => {
    ...
    // Event delegation for prompt table action buttons (icon buttons)
    document.getElementById("promptsTableBody").addEventListener("click", (e) => {
      ...
      if (btn.classList.contains("edit-icon-btn")) editPrompt(id);
      else if (btn.classList.contains("delete-icon-btn")) deletePrompt(id);
    });

    // Listen for default toggle changes
    document.getElementById("promptsTableBody").addEventListener("change", (e) => {
      const chk = e.target.closest("input.default-toggle[data-prompt-id]");
      if (!chk) return;
      const id = parseInt(chk.dataset.promptId, 10);
      // When toggled, call setDefault to update server and re-render
      setDefault(id);
    });
    ...
    document.getElementById("documentsTableBody").addEventListener("click", (e) => {
      const btn = e.target.closest("button.doc-delete-btn[data-doc-id]");
      if (!btn) return;
...
prompts.forEach(prompt => {
      ...
      // always keep storage in sync for default prompt
      if (prompt.is_default) {
        chrome.storage.local.set({ selected_prompt_id: prompt.id });
      }

      // Render row with the default toggle in the Default column and icons in Actions
      row.innerHTML = `
        <td>${escapeHtml(prompt.name)}</td>
        <td>${escapeHtml(prompt.description || "—")}</td>
        <td>${createdDate}</td>
        <td>
          <label class="toggle-switch" title="Set as Default">
            <input type="checkbox" class="default-toggle" data-prompt-id="${prompt.id}" ${prompt.is_default ? "checked" : ""}>
            <span class="toggle-slider"></span>
          </label>
        </td>
        <td>
          <div class="action-icon-group">
            <button class="icon-btn edit-icon-btn" title="Edit Prompt" data-prompt-id="${prompt.id}">
              <i class="fas fa-pencil-alt"></i>
            </button>
            <button class="icon-btn delete-icon-btn" title="Delete Prompt" data-prompt-id="${prompt.id}">
  ...
  try {
    ...
    docs.forEach(doc => {
      ...
      row.innerHTML = `
        ...
        <td><button class="doc-delete-btn" title="Delete Document" data-doc-id="${doc.id}"><i class="fas fa-trash"></i></button></td>
      `;
      tbody.appendChild(row);
    });
  } catch (err) {
